...
class FlvPlayer {
	...
	private function initPlayer() {
		...
		this.ns.onStatus = function(streamObj:Object) {
			switch (streamObj.code) {
				...
				case "NetStream.Play.StreamNotFound" :
					...
					_root.infoText.txt.text = "Unable to load file";
					break;
		...
		if (this.checkVersion(9)) {
			...
			_root.controller.rt.fullscreenBar.onRollOver = function(){
				...
				_root.infoTxt.txt.text = 'Fullscreen mode requires Flash Player 9.0.28+';
			}
			_root.controller.rt.fullscreenBar.onRollOut = function(){
				_root.infoTxt._visible = false;
				_root.infoTxt.txt.text = '';
			}
		}
	...
	private function buildPlayer(fs:Boolean) {
		...
		//Video Container
		_root.videoContainer._width = sw;
		_root.videoContainer._height = h;
		//VideoBg
		...
		//Text  
		_root.infoTxt._x = (sw-_root.infoTxt._width)/2;
		_root.infoTxt._y = (h-_root.infoTxt._height)/2 + _root.bigPlay._height;
		//Big Play
		_root.bigPlay._x = (sw-_root.bigPlay._width)/2;
		_root.bigPlay._y = (h-_root.bigPlay._height)/2;
		//Spinner
		_root.spinner._x = (sw-_root.spinner._width)/2;
		_root.spinner._y = (h-_root.spinner._height)/2;
		...
		if (this.getExtension(src) == 'xml') {
			...
			playlist.onLoad = function(success) {
				if (success) {
					...
					_root.spinner._visible = false;
					_root.infoText.text = "Unable to load file!";
				}
			};
		} else {
			this.movie = src;
	...
	private function checkVersion(required:Number) {
		var v:String = getVersion();
		var n:Array = v.split(",");
		var m:Number = n[0].substr(-1, n[0].indexOf(" "));
